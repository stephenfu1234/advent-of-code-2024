...
def solve(filename):
    ...
    print(sum(required_savings), 'cheats with at least 100 picoseconds')
...
def find_shortcuts(maze, visited):
    ...
    filtered_pairs = list(filter(lambda visited_location: abs(visited_location[0][2]-visited_location[1][2]) > 2, visited_pairs))
    ...
    filtered_pairs = list(filter(lambda visited_location: abs(visited_location[0][0]-visited_location[1][0]) + \
                                                          abs(visited_location[0][1]-visited_location[1][1]) <= 2, filtered_pairs))
    # print(len(filtered_pairs))
    
    time_savings = []
    for pair in filtered_pairs:
        saving = abs(pair[0][2] - pair[1][2]) - 2
